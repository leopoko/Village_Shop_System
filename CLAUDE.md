...
- レートはバニラの取引内容（エメラルド→物）を参照、値段はデフォルトx1.2
...
### 椅子の実装
- 不可視の ArmorStand を階段ブロック上にスポーンし、村人を騎乗させる
- Marker フラグで当たり判定を消す
- 休憩終了後に ArmorStand を discard
...
### config/
```
ModConfig.java                  # 静的フィールドによるConfig（Cloth Config未統合）
```
...
### 価格ソース（優先順）
1. **バニラ取引テーブル**: `TradeRegistry` のハードコード + `VillagerTrades.TRADES` スキャン
2. **食料価格**: `nutrition * count * foodPricePerHunger` (デフォルト: 0.25 = 栄養4で1エメラルド)
3. **ツール価格**: `durability * toolDurabilityFactor + speed * toolSpeedFactor`
4. 上記いずれにも該当しないアイテムは取引不可
...
### 購入価格
- バニラ買い取引あり: `ceil(baseEmeralds * purchasePriceMultiplier)` (x1.2)
- バニラ売り取引のみ: `ceil(baseEmeralds * purchasePriceMultiplier * 1.5)` (x1.8)
...
| 購入マークアップ | `purchasePriceMultiplier` | 1.2 |
...
- `ArmorStand.setMarker()` は private。NBT save/load で Marker フラグを設定
...
## 未実装・TODO
- [ ] Cloth Config 統合（現在は静的フィールドのみ）
- [ ] 村人が自分の職業と一致するアイテムの売却ペナルティ免除
- [ ] ポーション取引（良い効果の種類・強さで価格決定）
- [ ] config で追加取引アイテム設定
- [ ] 複数村人の同時棚操作時のロック機構
